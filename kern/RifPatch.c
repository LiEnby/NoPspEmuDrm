...
int is_offical_rif(PspRif* rif){
	...
	if(strcmp(rif->content_id, "JA0003-PCSC80018_00-POCKETSTATION001") == 0 && check_pksuite_installed()) {
	...
	if(!is_ecdsa_all_ff(rif->ecdsa_signature)) {
		LOG("[NOPSPEMUDRM_KERN] is_offical_rif -> true (ecdsa_signature != 0xff)\n");
		return 1;
	}
	LOG("[NOPSPEMUDRM_KERN] is_offical_rif -> false (ecdsa_signature == 0xff)\n");
	return 0;
}
...
static int ksceNpDrmGetRifInfoPatched(PspRif *psprif, int license_size, int mode, char *content_id, uint64_t *aid, uint16_t *license_version, uint8_t *license_flags, uint32_t *flags, uint32_t *sku_flag, uint64_t *start_time, uint64_t *expiration_time, uint64_t *flags2) {
	...
	if (ret < 0 &&
		...
		if (content_id) // copy content id from rif 
		  memcpy(content_id, psprif->content_id, 0x30);
		
		
		if (flags) {
		  if (__builtin_bswap16(psprif->license_type) & 0x200)
			(*flags) |= 0x1;
		  if (__builtin_bswap16(psprif->license_type) & 0x100)
			(*flags) |= 0x10000;
		}
		...
		if (license_flags)
		  *license_flags = (uint8_t)__builtin_bswap16(psprif->license_type);
